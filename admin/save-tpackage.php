  ...
  if($name_file != "") {
    ...
    if(in_array($ekstensi, $ekstensi_diperbolehkan) === true)  {
                  move_uploaded_file($file_tmp, 'package/'.$nama_gambar_baru); //memindah file gambar ke folder gambar
                   ...
                   $query  = "INSERT INTO package(id_package, name_package, segment_package, name_file, status_package) VALUES (NULL, '$name_package', '$segment_package', '$nama_gambar_baru', 'active')";
                    ...
                    if(!$result){
                      ...
                      echo "<script>alert('Data added successfully!');window.location='././Pack';</script>";
                    }
              } else {     
      ...
      // jalankan query UPDATE berdasarkan ID yang produknya kita edit
      $query  = "INSERT INTO package(id_package, name_package, segment_package, status_package) VALUES (NULL, '$name_package', '$segment_package', 'active')";
      $result = mysqli_query($db, $query);
      // periska query apakah ada error
      if(!$result){
            die ("Query gagal dijalankan: ".mysqli_errno($db).
